(function() {
    ...
    6: [function(require, module, exports) {
        function resetToDefaults() {
            $(".categoryBox").removeClass("active"), $('.categoryBox[value="helmet"]').addClass("active"), $("#levelFilter").html("Lv. Range: All"), $("#tierFilter").html("Tier Filter: All"), tierFilterIDX = 0, levelFilterIDX = 0, tierFilter = "All", levelFilter = "All", $("#itemSearch").val(""), $("#filters").show(), setTimeout(function() {
                $.getJSON(config.url.api + "public_api.php?action=itemDB&category=helmet", function(e) {
                    data = e.items, renderItems()
                })
            }, 100)
        }

        function renderItems() {
            $("#itemResult").html('<div class="row"><div id="col1" class="col-xs-4 itemColumn"></div><div id="col2" class="col-xs-4 itemColumn"></div><div id="col3" class="col-xs-4 itemColumn"></div></div>');
            for (var e in data)
                if ("default" != data[e].name && !("All" != tierFilter && tierFilter != data[e].tier || "All" != levelFilter && (levelFilter < data[e].level || levelFilter - 20 >= data[e].level) || -1 != data[e].name.indexOf("GM's") || -1 != data[e].name.indexOf("Builder's"))) {
                    var t = "";
                    t = "null" == data[e].addedLore || null == data[e].addedLore ? "" : data[e].addedLore;
                    var a = "";
                    $.each(data[e], function(t, l) {
                        void 0 !== infoActual[t] && "undefined" !== infoActual[t] && "tier" !== t && "name" !== t && "addedLore" !== t && 0 !== l && "0-0" !== l && "null" !== l && null !== l && (a += "<li>" + formatInfo(t, l, data[e].identified) + " " + infoActual[t] + "</span></li>")
                    });
                    var l = "";
                    $.each(data[e], function(e, t) {
                        void 0 !== reqActual[e] && "undefined" !== reqActual[e] && "null" !== t && null !== t && 0 !== t && (l += "<li><strong>" + reqActual[e] + ":</strong> " + cleanValue(e, t) + "</li>")
                    });
                    var i = "";
                    $.each(data[e], function(e, t) {
                        void 0 !== basicActual[e] && "undefined" !== basicActual[e] && "null" !== t && null !== t && 0 !== t && "0-0" !== t && ("attackSpeed" == e ? i = "<li>" + cleanValue(e, t) + " " + basicActual[e].text + '</li></ul><ul class="item-basic">' + i : i += '<li><span style="color:#' + basicActual[e].color + '">' + basicActual[e].text + ": " + cleanValue(e, t) + "</span></li>")
                    });
                    var r = "";
                    if (data[e].material && "" != data[e].material) {
                        var s, n = data[e].material,
                            o = ""; - 1 != n.indexOf(":") ? (s = n.substring(0, n.indexOf(":")), o = n.substring(n.indexOf(":") + 1)) : s = n;
                        var c = ".item-" + s + ("" !== o ? "-" + o : "-0");
                        selectorExists(c) && (r = '<div class="ingameitem item-' + s + ("" !== o ? "-" + o : "-0") + '"></div>')
                    } else {
                        switch (data[e].type) {
                            case "Bow":
                        ...
                        switch (data[e].accessoryType) {
                            case "Bracelet":
                                ...
                                break;
                            case "Ring":
                                r = '<div class="ingameitem item-95-0"></div>';
                                break;
                            case "Necklace":
                                r = '<div class="ingameitem item-102-0"></div>'
                        }
                    }
                    $(getShortestColumn()).append('<div class="item item-' + data[e].tier + '"><div class="text-strong item-name">' + (r || "") + data[e].name + '</div><ul class="item-basic">' + i + '</ul><ul class="item-req">' + l + '</ul><ul class="item-info">' + a + "</ul>" + (0 != data[e].sockets ? '<ul class="item-powder"><li>[0/' + data[e].sockets + "] Powder Slot</li></ul>" : "") + '<ul class="item-tier"><li>' + data[e].tier + ' Item</li></ul><em class="item-lore">' + t + "</em></div>")
                }
            $(".item").length < 1 && $("#itemResult").html("<center><h5>No results found!</h5></center>")
        }

        function cleanValue(e, t) {
            t = t.toString();
            for (var a in speeds)
                if (t == a) return speeds[a];
            for (var a in reformats)
                if (a == e) return "-" != t.substring(0, 1) && (t = "+" + t), t + reformats[a].substring(1);
            return t
        }

        function formatInfo(e, t, a) {
            var l = "";
            l = a ? t + "" : t >= 0 ? Math.ceil(.3 * t) + "</strong> to <strong>" + Math.ceil(1.3 * t) : Math.ceil(.7 * t) + "</strong> to <strong>" + Math.ceil(1.3 * t);
            for (var i in reformats)
                if (i == e) {
                    "-" != l.substring(0, 1) && (l = "+" + l), -1 == reformats[i].indexOf("%") ? l += reformats[i].substring(1) : -1 != l.indexOf("</strong>") ? l = l.substring(0, l.indexOf("</strong>")) + "% " + l.substring(l.indexOf("</strong>")) + reformats[i].substring(1) : l += reformats[i].substring(1);
                    break
                }
            return l = t >= 0 ? '<span style="color:#089209"><strong>' + l + "</strong>" : '<span style="color:#9d270c"><strong>' + l + "</strong>"
        }
        ...
        function getShortestColumn() {
            var e = {
                ...
                t = "col1";
            for (var a in e) e[a] < e[t] && (t = a);
            return "#" + t
        }

        function selectorExists(e) {
            for (var t = 0; t < document.styleSheets.length; t++)
                for (var a = document.styleSheets[t], l = a.rules ? a.rules : a.cssRules, i = 0; i < l.length; i++)
                    if (l[i].selectorText == e) return !0;
            return !1
        }
        $ = jQuery = require("jquery"), require("./ext/bootstrap.min"), require("./ext/jquery.dataTables.min"), require("./ext/jquery.smoothscroll.min"), require("./ext/baseTemplate");
        var config = require("./publicConfig.json"),
            data = [],
            tierFilter = "All",
            levelFilter = "All",
            levelFilters = ["All", 20, 40, 60, 80, 100],
            tierFilters = ["All", "Normal", "Unique", "Rare", "Legendary", "Mythic", "Set"],
            tierFilterIDX, levelFilterIDX = 0;
        $(document).ready(function() {
            var e = "";
            setInterval(function() {
                var t = $("#itemSearch").val();
                "" == t && "" != e ? (e = t, $("#filters").show(), resetToDefaults()) : t != e && (e = t, $("#filters").hide(), $("#itemResult").html("<p>Searching...</p>"), $.getJSON(config.url.api + "public_api.php?action=itemDB&search=" + t, function(e) {
                    data = e.items, renderItems()
                }))
            }, 1e3), $('a[href="#item-guideModal"]').click(function() {
                resetToDefaults()
            }), $(".categoryBox").click(function() {
                $(".categoryBox").removeClass("active"), $(this).addClass("active"), $("#itemResult").html("<p>Searching...</p>"), $.getJSON(config.url.api + "public_api.php?action=itemDB&category=" + $(this).attr("value"), function(e) {
                    data = e.items, renderItems()
                })
            }), $("#levelFilter").click(function() {
                levelFilterIDX++, levelFilters[levelFilterIDX] || (levelFilterIDX = 0), levelFilter = levelFilters[levelFilterIDX], $("#levelFilter").html("Lv. Range: " + ("All" != levelFilter ? levelFilter - 19 + "-" + levelFilter : levelFilter)), renderItems()
            }), $("#tierFilter").click(function() {
                tierFilterIDX++, tierFilters[tierFilterIDX] || (tierFilterIDX = 0), tierFilter = tierFilters[tierFilterIDX], $("#tierFilter").html("Tier Filter: " + tierFilter), renderItems()
            }), console.info("Help core loaded.")
        });
        var reqActual = {
                level: "Lv. Min ",
                quest: "Quest Req",
                classRequirement: "Class Req",
            ...
            infoActual = {
                ...
                attackSpeedBonus: "Attack Speed Tier",
